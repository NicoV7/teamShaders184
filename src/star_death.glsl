...
float mandelbulb_SDF ( in vec3 pos )
{
    ...
    for (float i = 0.0; i < max_iterations; i++)
    {
        ...
        vec3 z_n = vec3(cos(n * t) * cos(n * p), sin(n * t) * sin(n * p), cos(n * t));
        z_n *= pow(r, n);
        z = z_n + pos;
    }
...
vec3 raymarch(in vec3 o, in vec3 d, in float min_t, in float max_t, in float min_thresh, in float max_thresh)
{
    ...
    for (float i = 0.; i < max_iterations; i++) 
    {
        ...
        if (dst < min_thresh)
        {
            ...
            vec3 norm = calculateNormal(p);
            vec3 lightDir = normalize(vec3(1.0, 1.0, 1.0));
            float diff = max(0.0, dot(norm, lightDir));
            vec3 len = lightDir - p;
            vec3 rofl = (vec3(0.3, 0.3, 0.3) * diff) / (length(len) * length(len));
            color = mix(vec3(0.902,0.553,0.157), vec3(0.659,0.000,0.196), diff); //replace diff with rofl
            break;
        } else if (dst > max_thresh) {
            color = mix(vec3(0.129,0.012,0.055), vec3(0.925,0.682,0.957), 0.01);
            break;
        }
    }
    ...
    {
        color = vec3(0.039,0.067,0.902); // Assign a different color if no intersection was found within the steps
    }
