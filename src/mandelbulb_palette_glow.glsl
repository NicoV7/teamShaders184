...
vec3 palette( float t ) {
    //can be adjusted to the color gradient we want
    vec3 a = vec3(0.5, 0.5, 0.5);
    vec3 b = vec3(0.5, 0.5, 0.5);
    vec3 c = vec3(1.0, 1.0, 1.0);
    vec3 d = vec3(0.263,0.416,0.557);

    return a + b*cos( 6.2*(c*t+d) ); //we can adjust saturation
    //changing to sin or tan has interesting effects
}
...
vec3 raymarch(in vec3 o, in vec3 d, in float min_t, in float max_t, in float min_thresh, in float max_thresh, vec3 r)
{
    float t = min_t;
    ...
    for (float i = 0.; i < max_iterations; i++) 
    {
        ...
        if (dst < min_thresh)
        {
            vec3 p = curr; // Point of intersection
            ...
            //calculate the glow factor we want, can be adjusted 
            float glowfactor = clamp(1.0 - length(r) * 0.9, 0.0, 1.0);
            //the color we want
            vec3 glowcolor = vec3(0.918,0.800,0.941); 
            vec3 glow = glowfactor * glowcolor;
            
    
            color = palette(length(r) + 0.9 + iTime*.2) + glow;
            //add glow to palette, the iTime can be adjusted
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // raymarch
    vec3 col = raymarch(ro, d, min_t, max_t, min_thresh, max_thresh, vec3(0.0));
